...
class _FUIModalState extends State<FUIModal> {
  ...
  double _discernWidth() {
    if (widget.width != null) {
      ...
      if (width <= UIScreenSize.mobile) {
        return width * 0.8; // 80%
      } else if (width <= UIScreenSize.tablet) {
        return width * 0.7; // 70%
      } else if (width <= UIScreenSize.desktopFullHd) {
        return width * 0.6; // 60%
      } else if (width <= UIScreenSize.desktop4k) {
        return width * 0.4; // 40%
      } else {
  ...
  Widget _buildFooter() {
    if (widget.footerButtons != null && widget.footerButtons!.isNotEmpty) {
      ...
      for (int i = 0; i < widget.footerButtons!.length; i++) {
        if (i > 0) {
          rowWList.add(UISpacer.hSpace20);
        }
